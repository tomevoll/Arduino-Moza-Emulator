...
void I2CHandler::updateTxBufferForState(SlaveState state) {
    switch (state) {
        ...
        case F9_RECEIVED:
            ...
            break;
        case DD_RECEIVED: {
            // Queue single payload byte corresponding to btnSeqIdx_ (0-4)
            size_t idx = (btnSeqIdx_ >= 0) ? (static_cast<size_t>(btnSeqIdx_) % 5) : 0;
            xfer_.txBuf[0] = static_cast<char>(wheelState_.getButtonPayload(idx));
            xfer_.txCnt = 1;
            break;
        }
        case DE_RECEIVED: {
            // Queue single paddle payload byte corresponding to btnSeqIdx_ (0-4)
            size_t idx = (btnSeqIdx_ >= 0) ? (static_cast<size_t>(btnSeqIdx_) % 5) : 0;
            xfer_.txBuf[0] = static_cast<char>(wheelState_.getPaddlePayload(idx));
            xfer_.txCnt = 1;
            break;
        }
        case DB_RECEIVED:
